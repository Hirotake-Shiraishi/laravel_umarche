...
ユーザーが商品を検索・購入し**注文履歴**を確認し、購入済み商品には**レビュー（評価・コメント）**を投稿でき、オーナーが商品/在庫/画像/店舗情報に加え**受注（ステータス）**および**自店舗商品のレビュー閲覧**を行える、マルチログイン対応のEC風アプリケーションです。
...
- **運用者向け管理**（商品CRUD、在庫の増減、商品画像、店舗情報、**受注一覧・発送ステータス**、**レビュー一覧（自店舗分）**）
- **商品レビュー**（購入者のみ投稿・1 商品 1 ユーザー 1 件、平均評価の表示、オーナーは自店商品のレビューのみ閲覧）
- **レスポンシブ UI**（Tailwind CSS で主要画面をスマートフォン幅を含めレイアウト調整）
...
- **商品詳細**: 複数画像スライダー（Swiper）、在庫に応じた購入数量選択。**レビュー**は平均評価・一覧表示、**当該商品を購入済みのユーザー**のみ星・コメントを投稿可能（重複は DB の `unique(product_id, user_id)` でも防止）。販売停止などで一覧に載らない商品でも、当該ユーザーが過去購入していれば詳細・レビュー閲覧可（それ以外は 404）
  - 例: [`app/Http/Controllers/User/ItemController.php`](app/Http/Controllers/User/ItemController.php), [`app/Http/Controllers/User/ReviewController.php`](app/Http/Controllers/User/ReviewController.php), [`app/Http/Requests/ReviewStoreRequest.php`](app/Http/Requests/ReviewStoreRequest.php), [`resources/views/user/show.blade.php`](resources/views/user/show.blade.php), [`database/migrations/2026_04_18_021438_create_reviews_table.php`](database/migrations/2026_04_18_021438_create_reviews_table.php)
- **レビュー投稿**: `POST /show/{item}/reviews`（ルート名 `user.items.reviews.store`）。購入判定は Webhook で作成される `orders` / `order_items` を基準に `User::hasPurchasedProduct()` で統一
  - 例: [`routes/web.php`](routes/web.php), [`app/Models/Review.php`](app/Models/Review.php)
- **カート**: 追加/削除、小計計算
...
- **レビュー一覧**: 自店舗商品に付いたレビューのみを一覧（ページネーション）。商品名・評価・コメント・投稿者・日時を表示
  - 例: [`app/Http/Controllers/Owner/ReviewController.php`](app/Http/Controllers/Owner/ReviewController.php), [`resources/views/owner/reviews/index.blade.php`](resources/views/owner/reviews/index.blade.php), [`routes/owner.php`](routes/owner.php)
...
- **商品レビュー**: 「購入済み」の正は Webhook で作成される `orders` / `order_items`。投稿可否は `User::hasPurchasedProduct()` で判定し、`reviews` テーブルの `(product_id, user_id)` UNIQUE で二重投稿を DB でも防止。商品詳細の閲覧可否（掲載中 or 当該ユーザーの購入済み）は `ItemController` のミドルウェアと `ReviewController::store` の双方で確認
  - 例: [`app/Http/Controllers/User/ItemController.php`](app/Http/Controllers/User/ItemController.php), [`app/Http/Controllers/User/ReviewController.php`](app/Http/Controllers/User/ReviewController.php), [`app/Models/User.php`](app/Models/User.php)
...
- **商品レビュー**: 購入者のみ投稿・非購入拒否・非掲載商品と購入済み閲覧・オーナーは自店レビューのみ・重複投稿防止 等
  - [`tests/Feature/ProductReviewTest.php`](tests/Feature/ProductReviewTest.php)

テスト実行: `php artisan test` または `vendor/bin/phpunit`（**PHPUnit 9**、`composer.json` の require-dev）。**Feature 一式**は `php artisan test --testsuite=Feature`（現状 29 件）。Webhook 用のダミー値は [`phpunit.xml`](phpunit.xml) を参照。
